...
public class ficha : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //Seleccion de Puzzle
        if (mystats.alreadyBegun == true)
        {
            int temporalpuzzle = mystats.puzzles.puzzleID;
            Puzzles temp = puzzles[temporalpuzzle].GetComponent<Puzzles>();
            for (int z = 0; z < texpieces.Length; z++)
            {
                texpieces[z] = temp.pieces[z];
            }
            for (int x = 0; x < 16; x++)
            {
                if (mystats.puzzles.pieces[x] == true)
                {
                    piezas[x].mainTexture = texpieces[x];
                    llenoornot[x] = true;
                }
            }
            for (int y = 0; y < mystats.puzzles.maxDeckValue; y++)
            {
                cola.Enqueue(mystats.puzzles.deck[y]);
            }
            for (int a = 0; a < 4; a++)
            {
                if (mystats.puzzles.drawer[a, 0] != 100)
                {
                    cajon[a].mainTexture = texpieces[(int)mystats.puzzles.drawer[a, 0]];
                    rotcajon[a] = mystats.puzzles.drawer[a,1];
                    idscajon[a] = (int)mystats.puzzles.drawer[a, 0];
                    fullorempty[a] = true;
                }
            }
            followingPiece.mainTexture = texpieces[(int)mystats.puzzles.npiece[0]];
            rotnextpiece.transform.eulerAngles = new Vector3(0f, 0f, mystats.puzzles.npiece[1]);
            crot = mystats.puzzles.npiece[1];
            aux = (int)mystats.puzzles.npiece[0];
            actualPiece.mainTexture = texpieces[(int)mystats.puzzles.cpiece[0]];
            rotcurrentpiece.transform.eulerAngles = new Vector3(0f, 0f, mystats.puzzles.cpiece[1]);
            actualRot = mystats.puzzles.cpiece[1];
            if (mystats.puzzles.cpiece[0] == 100) {
                actualPiece.mainTexture = empty;
            }
            puzzleprogress = mystats.puzzles.myprogress;
            puzzleoppprogress = mystats.puzzles.myoppprogress;
            ...
            bucket.Query(query, (KiiQueryResult<KiiObject> result, Exception e) =>
            {
                ...
                {
                    obj.Delete((KiiObject deletedObj, Exception e1) =>
                    {
                        if (e1 != null)
                        {
                            // handle error
            ...
            if (mystats.aQuienLeTOCA == "2")
            {
                ...
                llena = true;
                firstsavegame = false;
                mystats.host = mystats.idplayer2;
            }
            else
            {
                fichei();
        ...
        else
        {
            ...
            deck();
            //Indica como debe iniciar cada jugador
            if (jugador == "2")
            {
                meToca = false;
                popUp.SetActive(true);
    ...
    void Update()
    {
        ...
        {
            StartCoroutine(checkFullMatch());
        }
        tiempo = tiempo + Time.deltaTime;
        tiempo2 = tiempo2 + Time.deltaTime;
        if (meToca == false && tiempo2 >= 15.0f)
        {
    ...
    IEnumerator checkFullMatch() //Verifica si la partida esta llena
    {
        activarllena = false;
        //idjson = StorageResponse.id;
        //storageService = sp.BuildStorageService();
         ...
         obj.Refresh((KiiObject obj2, Exception e) =>
            {
                ...
                {
                    mystats.groupURL = (string)obj2[key12]; //Coje el group Uri
                    value4 = (string)obj2[key4];
                    value5 = (string)obj2[key5];
                    Debug.Log("Verificando si la partida esta llena...");
                }
        ...
        if (value4 == "true")
        {
            mystats.idplayer2 = value5;
            //queueService = sp.BuildQueueService();
            //queueService.CreatePullQueue(mystats.idjson + mystats.idplayer + "Queue", "Queue Player1", callMessage);
            //queueService.CreatePullQueue(mystats.idjson + mystats.idplayer + "Pieces", "Pieces Player1", piecesMessage);
            llena = true;
            value12 = mystats.groupURL;
            //storageService = sp.BuildStorageService();
            //storageService.DeleteDocumentById(dbName, collectionName, idjson, callBack);
            //storageService.FindDocumentById(dbName, mystats.idplayer + "'s Games", mystats.yourIdJson, callBack);
            saveGame();
        }
        else
        {
